---
title: "ve-Venda (Tshivenḓa) - translation progress for f24"
date: 2026-01-17T15:28:07+00:00
code: ve
name_english: Venda
name_local: Tshivenḓa
progress: 12.68
progress_d: 0.25
release: f24
territories:
 - ZA
 - ZW
polib_error_count: 0
could_not_process_count: 340
---

Language progress for Venda (ve) in Fedora f24 is:

* 12.68% when we only look on started packages for this language.
* 0.25% when we compare to every single translatable string in Fedora f24.

Possible scripts are: Latn

* Total translatable string in Fedora f24: 14211507
* Source words to translate in started packages: 285647
* Translated words: 36206


Download:




* [ve generated stats (638 Bytes)]({{% resource url="/f24/ve.csv" %}})



Packages:

| Name |  Translated words | Total source words | Progress (%) | Language teams |
|------|------------------:|-------------------:|-------------:|----------------|
 | [libreoffice]({{< ref "/f24/package/libreoffice.md" >}}) | 33469 | 101018 | 33.1 | LANGUAGE <LL@li.org> | 
 | [tuxpaint]({{< ref "/f24/package/tuxpaint.md" >}}) | 475 | 1591 | 29.9 | LANGUAGE <LL@li.org> | 
 | [koffice-kivio]({{< ref "/f24/package/koffice-kivio.md" >}}) | 1896 | 8417 | 22.5 | <en@li.org> | 
 | [iso-codes]({{< ref "/f24/package/iso-codes.md" >}}) | 319 | 22357 | 1.4 | Venda <venda@translate.org.za> | 
 | [calibre]({{< ref "/f24/package/calibre.md" >}}) | 47 | 152264 | 0.0 | Venda (http://www.transifex.com/calibre/calibre/language/ve/) | 
{.sortable}


# Errors on PO files




## Error with calcstats

We use the calcstats from translate toolkit to get the translation progress, but sometimes it fails, here are the files:

| Package | Lang from file path/name | Team |
|---------|--------------------------|------|
{.sortable}


