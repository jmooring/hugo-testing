---
title: "he_IL-Hebrew (Israel) (עברית (ישראל)) - translation progress for f10"
date: 2026-01-17T07:56:32+00:00
code: he_IL
name_english: Hebrew (Israel)
name_local: עברית (ישראל)
progress: 36.76
progress_d: 0.23
release: f10
territories:
 - IL
polib_error_count: 0
could_not_process_count: 7
---

Language progress for Hebrew (Israel) (he_IL) in Fedora f10 is:

* 36.76% when we only look on started packages for this language.
* 0.23% when we compare to every single translatable string in Fedora f10.

Possible scripts are: Hebr

* Total translatable string in Fedora f10: 5351743
* Source words to translate in started packages: 32979
* Translated words: 12122


Download:




* [he_IL generated stats (731 Bytes)]({{% resource url="/f10/he_IL.csv" %}})



Packages:

| Name |  Translated words | Total source words | Progress (%) | Language teams |
|------|------------------:|-------------------:|-------------:|----------------|
 | [gnomesword]({{< ref "/f10/package/gnomesword.md" >}}) | 2415 | 2415 | 100.0 | GnomeSword Translation Team | 
 | [xiphos]({{< ref "/f10/package/xiphos.md" >}}) | 2743 | 2743 | 100.0 | Xiphos Translation Team | 
 | [exaile]({{< ref "/f10/package/exaile.md" >}}) | 1165 | 1333 | 87.4 | Hebrew <LL@li.org> | 
 | [filezilla]({{< ref "/f10/package/filezilla.md" >}}) | 3077 | 6860 | 44.9 | ABZPMGDNHM7 (please do not remove) | 
 | [squirrelmail]({{< ref "/f10/package/squirrelmail.md" >}}) | 2440 | 6878 | 35.5 | HEBREW <squirrelmail-i18n@lists.sourceforge.net> | 
 | [horde]({{< ref "/f10/package/horde.md" >}}) | 209 | 7103 | 2.9 | i18n@lists.horde.org | 
 | [imp]({{< ref "/f10/package/imp.md" >}}) | 73 | 5647 | 1.3 | i18n@lists.horde.org | 
{.sortable}


# Errors on PO files




## Error with calcstats

We use the calcstats from translate toolkit to get the translation progress, but sometimes it fails, here are the files:

| Package | Lang from file path/name | Team |
|---------|--------------------------|------|
{.sortable}


