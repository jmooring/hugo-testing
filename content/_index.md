...
Let's rotate it, change the layout engine to `elk`, and use a different light theme:
...
Let's change the layout engine to `tala`:

````text
```d2svg {layoutEngine=tala lightTheme=Origami}
direction: right
{{%/* generate-d2-site-structure-markup maxDepth=2 */%}}
```
````
