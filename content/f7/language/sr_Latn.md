---
title: "sr_Latn-Serbian (Latin) (Srpski (Latinica)) - translation progress for f7"
date: 2026-01-17T07:12:32+00:00
code: sr_Latn
name_english: Serbian (Latin)
name_local: Srpski (Latinica)
progress: 99.87
progress_d: 4.11
release: f7
territories:
 - not-found-in-cldr
polib_error_count: 0
could_not_process_count: 17
---

Language progress for Serbian (Latin) (sr_Latn) in Fedora f7 is:

* 99.87% when we only look on started packages for this language.
* 4.11% when we compare to every single translatable string in Fedora f7.

Possible scripts are: Latn

* Total translatable string in Fedora f7: 3457836
* Source words to translate in started packages: 142132
* Translated words: 141947


Download:




* [sr_Latn generated stats (746 Bytes)]({{% resource url="/f7/sr_Latn.csv" %}})



Packages:

| Name |  Translated words | Total source words | Progress (%) | Language teams |
|------|------------------:|-------------------:|-------------:|----------------|
 | [exaile](/f7/package/exaile) | 2355 | 2355 | 100.0 | Serbian <gnome@prevod.org> | 
 | [pirut](/f7/package/pirut) | 803 | 803 | 100.0 | Serbian (sr) <fedora-trans-sr@redhat.com> | 
 | [wesnoth](/f7/package/wesnoth) | 119063 | 119067 | 100.0 | Serbian | 
 | [libvirt](/f7/package/libvirt) | 2331 | 2337 | 99.7 | Serbian (sr) <fedora-trans-sr@redhat.com> | 
 | [pidgin](/f7/package/pidgin) | 15667 | 15731 | 99.6 | Serbian <gnome@prevod.org> | 
 | [isomaster](/f7/package/isomaster) | 654 | 672 | 97.3 |  | 
 | [smolt](/f7/package/smolt) | 744 | 803 | 92.7 | Serbian (sr) <fedora-trans-sr@redhat.com> | 
 | [asunder](/f7/package/asunder) | 330 | 364 | 90.7 |  | 
{.sortable}


# Errors on PO files




## Error with calcstats

We use the calcstats from translate toolkit to get the translation progress, but sometimes it fails, here are the files:

| Package | Lang from file path/name | Team |
|---------|--------------------------|------|
{.sortable}


