---
title: "es_PA-Spanish (Panama) (Español (Panamá)) - translation progress for f16"
date: 2026-01-20T02:38:28+00:00
code: es_PA
name_english: Spanish (Panama)
name_local: Español (Panamá)
progress: 98.26
progress_d: 0.05
release: f16
territories:
 - PA
polib_error_count: 0
could_not_process_count: 3
---

Language progress for Spanish (Panama) (es_PA) in Fedora f16 is:

* 98.26% when we only look on started packages for this language.
* 0.05% when we compare to every single translatable string in Fedora f16.

Possible scripts are: Latn

* Total translatable string in Fedora f16: 9698971
* Source words to translate in started packages: 4783
* Translated words: 4700


Download:




* [es_PA generated stats (408 Bytes)]({{% resource url="/f16/es_PA.csv" %}})



Packages:

| Name |  Translated words | Total source words | Progress (%) | Language teams |
|------|------------------:|-------------------:|-------------:|----------------|
 | [gnome-applets]({{< ref "/f16/package/gnome-applets.md" >}}) | 3858 | 3858 | 100.0 | Spanish <traductores@es.gnome.org> | 
 | [libgweather]({{< ref "/f16/package/libgweather.md" >}}) | 447 | 460 | 97.2 | Spanish <traductores@es.gnome.org> | 
 | [libmateweather]({{< ref "/f16/package/libmateweather.md" >}}) | 395 | 465 | 84.9 | LANGUAGE <LL@li.org> | 
{.sortable}


# Errors on PO files




## Error with calcstats

We use the calcstats from translate toolkit to get the translation progress, but sometimes it fails, here are the files:

| Package | Lang from file path/name | Team |
|---------|--------------------------|------|
{.sortable}


