...
import (
	"context"
	"dagger/hugo-github-issue-12314/internal/dagger"
	"encoding/json"
	"fmt"
	"os"
)
...
func (r HugoGithubIssue12314) MarshalJSON() ([]byte, error) {
	var concrete struct{}
	return json.Marshal(&concrete)
}

func (r *HugoGithubIssue12314) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func (r HugoGithubIssue12314Builder) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Container *Container
	}
...
func (r *HugoGithubIssue12314Builder) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Container *Container
	}
	err := json.Unmarshal(bs, &concrete)
...
func (r HugoGithubIssue12314Build) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Directory *Directory
	}
...
func (r *HugoGithubIssue12314Build) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Directory *Directory
	}
	err := json.Unmarshal(bs, &concrete)
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	switch parentName {
	case "HugoGithubIssue12314":
		switch fnName {
		case "Builder":
			var parent HugoGithubIssue12314
			...
			return (*HugoGithubIssue12314).Builder(&parent, directory), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "HugoGithubIssue12314Builder":
		switch fnName {
		case "Build":
			var parent HugoGithubIssue12314Builder
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			...
			return (*HugoGithubIssue12314Builder).Build(&parent, args), nil
		case "Server":
			var parent HugoGithubIssue12314Builder
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*HugoGithubIssue12314Builder).Server(&parent), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("HugoGithubIssue12314").
					WithFunction(
						dag.Function("Builder",
							dag.TypeDef().WithObject("HugoGithubIssue12314Builder")).
							WithArg("directory", dag.TypeDef().WithObject("Directory")))).
			WithObject(
				dag.TypeDef().WithObject("HugoGithubIssue12314Builder").
					WithFunction(
						dag.Function("Build",
							dag.TypeDef().WithObject("HugoGithubIssue12314Build")).
							WithArg("args", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)).WithOptional(true))).
					WithFunction(
						dag.Function("Server",
							dag.TypeDef().WithObject("Service"))).
					WithField("Container", dag.TypeDef().WithObject("Container"))).
			WithObject(
				dag.TypeDef().WithObject("HugoGithubIssue12314Build").
					WithField("Directory", dag.TypeDef().WithObject("Directory"))), nil
	default:
